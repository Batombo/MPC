...
def model():
    ...
    Discrete_Time_Model = scipy.io.loadmat('Discrete_Time_Model.mat')
    ...
    u_rad_OfficesZ2 = SX.sym("u_rad_OfficesZ2")

    u_AHU1_noERC = SX.sym("u_AHU1_noERC")
    u_AHU2_noERC = SX.sym("u_AHU2_noERC")

    u_blinds_S = SX.sym("u_blinds_S")
    u_blinds_W = SX.sym("u_blinds_W")
    u_blinds_N = SX.sym("u_blinds_N")
    u_blinds_E = SX.sym("u_blinds_E")

    u = vertcat(u_blinds_E, u_blinds_N, u_blinds_S, u_blinds_W, u_AHU1_noERC, u_AHU2_noERC, u_rad_OfficesZ1, u_rad_OfficesZ2)
    ...
    x0 = 22*NP.ones(36)
    x0[[4, 5, 10, 11, 14, 15, 18, 19, 21, 22, 25, 26, 29, 30, 33, 34]] = 5
    # No algebraic states
    z0 = NP.array([])
    ...
    u_lb = NP.array([0, 0, 0, 0, 0, 0, 0, 0])
    u_ub = NP.array([1, 1, 1, 1, 0.18328, 0.18328, inf, inf])
    u0 = NP.array([1, 1, 1, 1, 0, 0, 25, 25])

    # Scaling factors for the states and control inputs. Important if the system is ill-conditioned
    x_scaling = NP.ones(36)
    z_scaling = NP.array([])
