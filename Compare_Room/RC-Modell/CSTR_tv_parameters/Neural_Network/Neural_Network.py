...
u_AHU1_noERC_train = u_AHU1_noERC[0:int(0.8*len(u_AHU1_noERC))]
...
v_windspeed_train = v_windspeed[0:int(0.8*len(v_windspeed))]
...
sample n            .    |     .    |     .
"""
x_train = NP.transpose(NP.squeeze(NP.array([HeatRate_train, u_AHU1_noERC_train, ZoneTemp_train , OutdoorTemp_train, v_IG_Offices_train,
v_solGlobFac_E_train, v_solGlobFac_N_train, v_solGlobFac_S_train, v_solGlobFac_W_train, v_windspeed_train,
...
x_train = NP.delete(x_train, 3*numbers-1, 1)
if load == 0:
...
if load == 1:
    ...
    trained = model.fit(x_train, y_train, validation_split=0.1, shuffle = False, epochs=1 00, batch_size=256, verbose = 2)
...
x_test = NP.transpose(NP.squeeze(NP.array([HeatRate_test, u_AHU1_noERC_test, ZoneTemp_test ,OutdoorTemp_test, v_IG_Offices_test,
v_solGlobFac_E_test, v_solGlobFac_N_test, v_solGlobFac_S_test, v_solGlobFac_W_test, v_windspeed_test,
...
x_test = tmp[:]
# Delete the current ZoneTemp(t)
x_test = NP.delete(x_test, 3*numbers-1, 1)
...
rmse = NP.sqrt(NP.mean(( y_test-NP.squeeze(classes))**2))
print("RMSE: " + str(rmse))
...
fig2 = P.figure(2)
P.clf()
P.plot(y_test- NP.squeeze(classes))
P.ylabel('Difference')
P.show()
