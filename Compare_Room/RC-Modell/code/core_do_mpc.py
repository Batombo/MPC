...
class configuration:
    ...
    def step_simulator(self, model_fmu, simTime, secStep):
        ...
        tmp[0:2] = (~tmp[0:2].astype(bool)).astype(int)
        ...
        AHU
        """
        tmp[2] = converter.massflow2volumeflow(self.simulator.x0_sim[0], tmp[2])
        tmp[2] = converter.volumeflow2sched(tmp[2])
        model_fmu.set('u_AHU1_noERC', tmp[2])
        ...
        # Do 2 Simulation Steps
        if  (simTime -  daystart*60)/600 < 1:
            tmp[3] = x_next[0]
        else:
            ...
            v_IG_Offices = self.optimizer.tv_p_values[length-1:length+1,0,0]*63.15

            v_solGlobFac_E = self.optimizer.tv_p_values[length-1:length+1,3,0]
            v_solGlobFac_N = self.optimizer.tv_p_values[length-1:length+1,4,0]
            v_solGlobFac_S = self.optimizer.tv_p_values[length-1:length+1,5,0]
            v_solGlobFac_W = self.optimizer.tv_p_values[length-1:length+1,6,0]
            v_windspeed = self.optimizer.tv_p_values[length-1:length+1,7,0]

            u_blind_past = (~NP.round(self.mpc_data.mpc_control[length-1:length,0:2]).astype(bool)).astype(int)
            u_blinds_N = NP.append(u_blind_past[:,0], tmp[0])
            u_blinds_W = NP.append(u_blind_past[:,1], tmp[1])


            x_test = NP.concatenate((HeatRate, u_AHU1_noERC, ZoneTemp ,OutdoorTemp, v_IG_Offices,
                     v_solGlobFac_E, v_solGlobFac_N, v_solGlobFac_S, v_solGlobFac_W, v_windspeed,
        ...
        # Get the actual inputs from E+
        HeatRate = NP.zeros((1,1))
        for i in range(0, HeatRate.shape[0]):
            HeatRate[i,:] = model_fmu.get('HeatRate_Z'+str(i+1))
            self.optimizer.u_mpc[3] = HeatRate/63.15

        u_AHU1_noERC = NP.zeros((1,1))
        for i in range(0, u_AHU1_noERC.shape[0]):
            u_AHU1_noERC[i,:] = model_fmu.get('v_vent_'+str(i+1))
            self.optimizer.u_mpc[2] = u_AHU1_noERC

        self.optimizer.u_mpc[0] = (~model_fmu.get('u_blinds_N_val').astype(bool)).astype(int)
        self.optimizer.u_mpc[1] = (~model_fmu.get('u_blinds_W_val').astype(bool)).astype(int)
        ...
        states.append(model_fmu.get('t_faceout_101'))

        states.append(model_fmu.get('t_faceout_102'))
        states.append(NP.array(x_next[3]))
        states.append(NP.array(x_next[4]))
        states.append(model_fmu.get('t_facein_102'))

        states.append(model_fmu.get('t_faceout_106'))
        states.append(NP.array(x_next[7]))
        states.append(NP.array(x_next[8]))
        states.append(model_fmu.get('t_facein_106'))

        states.append(model_fmu.get('t_faceout_107'))
        states.append(NP.array(x_next[11]))
        states.append(NP.array(x_next[12]))
        states.append(model_fmu.get('t_facein_107'))

        states.append(model_fmu.get('t_faceout_108'))
        states.append(NP.array(x_next[15]))
        states.append(NP.array(x_next[16]))
        states.append(model_fmu.get('t_facein_108'))

        states.append(model_fmu.get('t_faceout_109'))
        states.append(NP.array(x_next[19]))
        states.append(NP.array(x_next[20]))
        states.append(model_fmu.get('t_facein_109'))

        states.append(model_fmu.get('t_faceout_110'))
        states.append(NP.array(x_next[23]))
        states.append(NP.array(x_next[24]))
        states.append(model_fmu.get('t_facein_110'))

        states.append(model_fmu.get('t_faceout_111'))
        states.append(NP.array(x_next[27]))
        states.append(NP.array(x_next[28]))
        states.append(model_fmu.get('t_facein_111'))

        states.append(model_fmu.get('t_faceout_116'))
        states.append(NP.array(x_next[31]))
        states.append(NP.array(x_next[32]))
        states.append(model_fmu.get('t_facein_116'))

        states.append(model_fmu.get('t_faceout_117'))
        states.append(NP.array(x_next[35]))
        states.append(model_fmu.get('t_facein_117'))

        states = NP.squeeze(states)

        if  (simTime -  daystart*60)/600 < 2:
            self.simulator.xf_sim = NP.squeeze(NP.array(x_next))
        else:
