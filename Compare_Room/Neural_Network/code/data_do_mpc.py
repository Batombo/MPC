...
def plot_mpc(configuration):
    ...
    plt.plot(mpc_time[0:index_mpc], HeatRate[0:index_mpc,plot_states[index]])
    ...
    plt.plot(mpc_time[0:index_mpc], faultDetector[0:index_mpc,plot_states[index]])
    plt.ylabel('faultDetector []')
    plt.xlabel("Time")
    plt.grid()
    axes = plt.gca()
    axes.set_xlim([mpc_time[0]-5,mpc_time[-1]+10*10])
...
def plot_animation(configuration):
    ...
    if configuration.simulator.plot_anim:
        ...
        # Plot Heatrate
        plot = plt.subplot(total_subplots, 1, len(plot_states) + index + 1)
        plt.plot(mpc_time[0:index_mpc], HeatRate[0:index_mpc,plot_states[index]],'-k', linewidth=2.0)
        plt.ylabel('HeatRate [W]')
        plt.xlabel("Time")
        plt.grid()
        axes = plt.gca()
